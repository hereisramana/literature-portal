...
export default function EmptyState({ suggestions = [], onSelectSuggestion }) {
  ...
  return (
    <div className="mx-auto max-w-lg px-6 py-10 md:py-16 text-center">
      <div className="mb-6 flex justify-center text-[var(--text-muted-color)]">
      ...
      <h2 className="text-[20px] font-bold text-[var(--color-text-strong)] mb-3">
        {hasSuggestions
      ...
      {hasSuggestions && (
        <div className="mt-8">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--text-muted-color)] mb-4">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {suggestions.map((s) => (
              <motion.button
                ...
                className="rounded-full bg-[var(--clr-surface)] border border-[var(--color-border-subtle)] px-4 py-2 text-[12px] font-medium text-[var(--text-body-color)] hover:bg-[var(--color-interaction-hover)] hover:border-[var(--color-border-strong)] transition"
              >
                {s.label} <span className="text-[var(--text-muted-color)] ml-1">{s.count}</span>
              </motion.button>
            ))}
          </div>
        </div>
